    ...
    >
      ...
      >
        ...
        <div className="px-7 pb-[26px] pt-[22px]">
          ...
          {/* Shown only when canEnquire() agrees: staff have switched the
              enquiry form on for the WHOLE directory (Strapi’s Dealer
              Directory Settings single type, #70), AND Connect has issued this
              particular dealer a company id (ETN-017), AND the dealer is
              accredited (Connect’s `approved` — set on the owner's first login
              after approval, captain's decision 2026-09-22). The switch
              arrives as a prop rather than on `dealer` precisely because it is
              not a property of any one dealer; the id and accreditation halves
              ride on `dealer` as derived booleans.
              ...
              3 implies 2: accreditation can only happen after approval, and
              Connect mints the id at approval. The reverse never holds — that
              gap is exactly the pre-accreditation state this gate exists for.
              ...
              for a dealer with no company id OR no accreditation, so what
              looks closed here really is closed. That is the same call ETN-013
              D3 made for the publish gate, and deliberately a stricter one
              than #70's. */}
