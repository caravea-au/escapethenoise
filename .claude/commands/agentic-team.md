...
1. **Repository discovery.** Inspect structure, scripts, frontend/backend stacks, styling system, and conventions (root `CLAUDE.md`, `frontend/CLAUDE.md`, `backend/CLAUDE.md`, `.claude/COMPONENTS.md`, the `nextjs-component-standards` skill). Do not edit yet. Produce a Repository Discovery report.
...
3. **Architecture planning.** Delegate to `frontend-architect` (component tree, reuse plan, state/data flow, styling/token strategy, a11y). Delegate to `backend-architect` whenever the design renders content (routes, controllers, services, content types, the shared **`seo` component** + **`navigation` global**, validation, auth, and **what content to seed**, test plan). Then convert plans into small tasks with acceptance criteria.
4. **Implementation (content lives in Strapi).** Delegate one task at a time to `frontend-developer` / `backend-developer` by area. Preserve existing functionality; reuse existing code, tokens, components, utilities, API clients, schemas, and Strapi patterns; avoid new dependencies unless clearly justified. **Seed content into local Strapi first** — extract copy/headings/images/SEO/collection entries from the Figma design (`backend-developer`, idempotent upsert; for an export-driven repo use `/seed`), then the frontend **fetches** it. **No hardcoded editorial content, SEO, or fallback defaults** — a missing field renders an empty/error state (that microcopy stays in code). Code-only exceptions: form fields, routing/nav tree, system microcopy. Commit each completed task as a checkpoint if git is available.
...
- **Content provenance:** all editorial content, imagery, and SEO come from Strapi (seeded, then fetched) — never hardcoded and never a hardcoded fallback. `design-fidelity-qa` / `functional-qa` enforce this in QA.
