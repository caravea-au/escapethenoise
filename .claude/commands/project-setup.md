...
2. **Content model (Strapi)** — FIRST read what already exists via the `strapi` MCP (`strapi_get_content_types`)
   and list it. Then interview for the rest: content types + fields + relationships (e.g. `Dealer`, `Event`,
   `Guide`, `StateAssociation`). Reconcile with the sitemap.
...
`--font-display`, `--font-body`, and `--font-serif` if the art direction calls for a serif accent (e.g.
Fraunces). Expose theme utilities in `tokens.css` (`font-oswald`, `font-sans`, `font-serif`).
