...
- Content provenance: confirm on-page content is **fetched from Strapi**, not hardcoded — SEO from the `seo` component (via `generateMetadata`), collection lists (events / stock / range / blogs) from Strapi entries not an inline array, header/footer nav from the `navigation` global; a missing/empty field shows an **empty/error state**, never a hardcoded default string.
